...
return false;})()},ScriptFragment:'<script[^>]*>([\\S\\s]*?)<\/script>',JSONFilter:/^\/\*-secure-([\s\S]*)\*\/\s*$/,emptyFunction:function(){},K:function(x){return x}};if(Prototype.Browser.MobileSafari)
Prototype.BrowserFeatures.SpecificElementExtensions=false;var Abstract={};var Try={these:function(){var returnValue;for(var i=0,length=arguments.length;i<length;i++){var lambda=arguments[i];try{returnValue=lambda();break;}catch(e){}}
return returnValue;}};var Class=(function(){function create(){var parent=null,properties=$A(arguments);if(Object.isFunction(properties[0]))
...
if(SELECT_ELEMENT_INNERHTML_BUGGY||TABLE_ELEMENT_INNERHTML_BUGGY){if(tagName in Element._insertionTranslations.tags){$A(element.childNodes).each(function(node){element.removeChild(node);});Element._getContentFromAnonymousElement(tagName,content.stripScripts()).each(function(node){element.appendChild(node)});}
...
var HTMLOBJECTELEMENT_PROTOTYPE_BUGGY=checkDeficiency('object');if(Prototype.BrowserFeatures.SpecificElementExtensions){if(HTMLOBJECTELEMENT_PROTOTYPE_BUGGY){return function(element){if(element&&element.tagName){var tagName=element.tagName.toUpperCase();if(tagName==='OBJECT'||tagName==='APPLET'){extendElementWith(element,Element.Methods);if(tagName==='OBJECT'){extendElementWith(element,Element.Methods.ByTag.OBJECT)}
else if(tagName==='APPLET'){extendElementWith(element,Element.Methods.ByTag.APPLET)}}}
return element;}}
return Prototype.K;}
...
Element.Storage[uid]=$H();return Element.Storage[uid];},store:function(element,key,value){if(!(element=$(element)))return;if(arguments.length===2){element.getStorage().update(key);}else{element.getStorage().set(key,value);}
...
var respondersForEvent=registry.get(eventName);if(Object.isUndefined()){respondersForEvent=[];registry.set(eventName,respondersForEvent);}
if(respondersForEvent.pluck('handler').include(handler))return false;var responder;if(eventName.include(":")){responder=function(event){if(Object.isUndefined(event.eventName))
return false;if(event.eventName!==eventName)
return false;Event.extend(event,element);handler.call(element,event);};}else{if(!Prototype.Browser.IE&&(eventName==="mouseenter"||eventName==="mouseleave")){if(eventName==="mouseenter"||eventName==="mouseleave"){responder=function(event){Event.extend(event,element);var parent=event.relatedTarget;while(parent&&parent!==element){try{parent=parent.parentNode;}
...
window.addEventListener('unload',Prototype.emptyFunction,false);var _getDOMEventName=Prototype.K;if(!Prototype.Browser.IE){_getDOMEventName=function(eventName){var translations={mouseenter:"mouseover",mouseleave:"mouseout"};return eventName in translations?translations[eventName]:eventName;};}
...
return elements;};return function(className,parentElement){return $(parentElement||document.body).getElementsByClassName(className);};}(Element.Methods);Element.ClassNames=Class.create();Element.ClassNames.prototype={initialize:function(element){this.element=$(element);},_each:function(iterator){this.element.className.split(/\s+/).select(function(name){return name.length>0;})._each(iterator);},set:function(className){this.element.className=className;},add:function(classNameToAdd){if(this.include(classNameToAdd))return;this.set($A(this).concat(classNameToAdd).join(' '));},remove:function(classNameToRemove){if(!this.include(classNameToRemove))return;this.set($A(this).without(classNameToRemove).join(' '));},toString:function(){return $A(this).join(' ');}};Object.extend(Element.ClassNames.prototype,Enumerable);var Builder={NODEMAP:{AREA:'map',CAPTION:'table',COL:'table',COLGROUP:'table',LEGEND:'fieldset',OPTGROUP:'select',OPTION:'select',PARAM:'object',TBODY:'table',TD:'table',TFOOT:'table',TH:'table',THEAD:'table',TR:'table'},node:function(elementName){elementName=elementName.toUpperCase();var parentTag=this.NODEMAP[elementName]||'div';var parentElement=document.createElement(parentTag);try{parentElement.innerHTML="<"+elementName+"></"+elementName+">";}catch(e){}
...
ipe._effect.cancel();},onFailure:function(transport,ipe){alert('Error communication with the server: '+transport.responseText.stripTags());},onFormCustomization:null,onLeaveEditMode:null,onLeaveHover:function(ipe){ipe._effect=new Effect.Highlight(ipe.element,{startcolor:ipe.options.highlightColor,endcolor:ipe.options.highlightEndColor,restorecolor:ipe._originalBackground,keepBackgroundImage:true});}},Listeners:{click:'enterEditMode',keydown:'checkForEscapeOrReturn',mouseover:'enterHover',mouseout:'leaveHover'}});Ajax.InPlaceCollectionEditor.DefaultOptions={loadingCollectionText:'Loading options...'};Form.Element.DelayedObserver=Class.create({initialize:function(element,delay,callback){this.delay=delay||0.5;this.element=$(element);this.callback=callback;this.timer=null;this.lastValue=$F(this.element);Event.observe(this.element,'keyup',this.delayedListener.bindAsEventListener(this));},delayedListener:function(event){if(this.lastValue==$F(this.element))return;if(this.timer)clearTimeout(this.timer);this.timer=setTimeout(this.onTimerEvent.bind(this),this.delay*1000);this.lastValue=$F(this.element);},onTimerEvent:function(){this.timer=null;this.callback(this.element,$F(this.element));}});var hibird={version:'1.2'};var log={clear:function(){},debug:function(){},warn:function(){},error:function(){},info:function(){},profile:function(){}};(function(){hibird.ready=function(f){document.observe("dom:loaded",f);};hibird.ready(function(){$$('html').first().addClassName('has-js');$$('.js').invoke('removeClassName','js');});})();(function(){var notifier={load:function(){$('info').observe('click',notifier.hide);var url=window.location.href.toString();if(url.indexOf('&info=')>=0||url.indexOf('?info=')>=0){$('info').hide();hibird.notify($('info').select('div').first().innerHTML);}},show:function(msg){$('info').hide().update("<div>"+msg+"</div>").blindDown({duration:0.3});},hide:function(delay){var delay=(delay==null)?0:delay;setTimeout("$('info').blindUp({duration: 0.3});",delay);},notify:function(msg,options){options=Object.extend({delay:10000,kind:null},options||{});$('info').className=options.kind||'';notifier.show(msg);notifier.hide(options.delay);}};hibird.notify=notifier.notify;hibird.ready(function(){notifier.load();});})();function TextAreaResizer(id,options){this.textarea=id;this.staticOffset;this.iLastMousePos=0;this.iMin=32;this.grip;this.options=options;this.init();};TextAreaResizer.prototype.init=function(){this.textarea.addClassName('processed')
this.staticOffset=null;var span=new Element('span');Element.wrap(this.textarea,span);span.wrap(new Element('div',{'class':'resizable-textarea'}));var grippie=new Element('div',{'class':'grippie'});span.parentNode.insert(grippie);Event.observe(grippie,'mousedown',this.startDrag.bindAsEventListener(this));};TextAreaResizer.prototype.startDrag=function(event){var data=$A(arguments);data.shift();this.textarea=$(Event.element(event)).previous().firstDescendant();this.iLastMousePos=event.pointerY();this.staticOffset=this.textarea.getHeight()-this.iLastMousePos;this.textarea.setStyle({'opacity':0.25});Event.observe(document,'mousemove',this.performDrag.bindAsEventListener(this));Event.observe(document,'mouseup',this.endDrag.bindAsEventListener(this));return false;};TextAreaResizer.prototype.performDrag=function(event){var data=$A(arguments);data.shift();var iThisMousePos=event.pointerY();var iMousePos=this.staticOffset+iThisMousePos;if(this.iLastMousePos>=(iThisMousePos)){iMousePos-=5;}
this.iLastMousePos=iThisMousePos;iMousePos=Math.max(this.iMin,iMousePos);this.textarea.setStyle({height:iMousePos+'px'});if(iMousePos<this.iMin){this.endDrag(event);}
return false;};TextAreaResizer.prototype.endDrag=function(event){var data=$A(arguments);data.shift();Event.stopObserving(document,'mousemove');Event.stopObserving(document,'mouseup');this.textarea.setStyle({'opacity':1});this.textarea.focus();this.staticOffset=null;this.textarea=null;this.iLastMousePos=0;if(this.options){if(this.options.afterDrag){this.options.afterDrag();}}};(function(){var effect=function(options,self,o,next){var effectName=this;o.each(function(i){if(i.visible()==options.whenVisible){i[effectName]({duration:0.3,afterFinish:function(){do_next(self,o,next);}})}else{do_next(self,o,next);}});};var toggle=function(showAction,self,o,next){var hideAction=this;if(o&&o[0].visible()){var action=hideAction;}else{var action=showAction;}
o.invoke(action,{duration:0.3,afterFinish:function(){do_next(self,o,next);}});};var toggleSelected=function(self,o,next){if(o&&o[0].hasClassName('selected')){var action='removeClassName';}else{var action='addClassName';}
o.invoke(action,'selected');do_next(self,o,next);};var do_next=function(self,o,next){if(next){var action=next.substring(0,(next+'|').indexOf('|'));next=next.substring(action.length+1);if(action.include(':')){var filter=action.substring(0,action.indexOf(':'));var action=action.substring(filter.length+1);o=scripty.select(self,filter);}
scripty.actions[action](self,o,next);}};var init_hint=function(input){var t=input;while(t&&t.tagName!='FORM'){t=t.parentNode}
var label=$(t.select('label[for='+input.name+']').first());Object.extend(label.style,{zIndex:10,position:'absolute',fontSize:input.getStyle('fontSize'),paddingTop:input.getStyle('paddingTop'),paddingLeft:input.getStyle('paddingLeft'),marginTop:input.getStyle('marginTop'),marginLeft:input.getStyle('marginLeft')});Object.extend(input.style,{zIndex:100});label.onclick=function(input){input.focus();}.bind(label,input);input.makePositioned();if(input.value.empty()){input.addClassName('empty');}};var manage_inline_form=function(dom){dom.select('.inline input').each(function(i){try{if(i.type!='text'&&i.type!='password')return;init_hint(i);i.observe('focus',function(e){var m=e.element();$(m).addClassName('focus');});i.observe('keypress',function(e){var m=e.element();$(m).removeClassName('empty');});i.observe('blur',function(e){var m=e.element();$(m).removeClassName('focus');if(m.value.empty()){$(m).addClassName('empty');}});}catch(e){}});};var manage_textarea=function(dom){dom.select('textarea').each(function(tb){new TextAreaResizer(tb);});};var scripty={actions:{show:function(self,o,next){o.invoke('show');do_next(self,o,next);},hide:function(self,o,next){o.invoke('hide');do_next(self,o,next);},clear:function(self,o,next){o.invoke('update');do_next(self,o,next);},reset:function(self,o,next){o.invoke('reset');do_next(self,o,next);o.each(function(f){f.select('input').each(function(m){$(m).removeClassName('focus');if(!$(m).value)$(m).addClassName('empty');});});},makeajax:function(self,o,next){o.each(function(f){f.select('form').each(function(form){if(form.hasClassName('ajax'))return;$(form).observe('submit',function(self,o,next,ev){scripty.actions.submit.bind(self)(self,o,next);ev.stop();return false;}.bind(self,self,o,next));});});},focus:function(self,o,next){},highlight:function(self,o,next){o.invoke('highlight');do_next(self,o,next);},save_ok:function(self,o,next){hibird.notify('Saved successfully');do_next(self,o,next);},appear:effect.bind('appear',{whenVisible:false}),fade:effect.bind('fade',{whenVisible:true}),blindUp:effect.bind('blindUp',{whenVisible:true}),blindDown:effect.bind('blindDown',{whenVisible:false}),blind:toggle.bind('blindUp','blindDown'),toggleSelected:toggleSelected,reload:function(self,o,next){var x=o;x.each(function(item){if(item.className.include('PARTIAL')){if(item.title){var target=item.title;$(target).store('target',target);this.title='';}else{var target=$(item).retrieve('target');}
if(target){var chunks=target.split('#',2);var url=chunks[0];var partName=chunks[1];}else{var url=window.location.href.toString().split('#',1)[0];var partName=item.className.split('PARTIAL ',2)[1].split(' ',2)[0];}
if(url.include('?')){url+='&PARTIAL=';}else{url+='?PARTIAL=';}
new Ajax.Request(url+partName,{method:'get',evalScripts:false,evalJS:false,onSuccess:function(self,o,next,trans){this.update(trans.responseText);scripty.hook(this);do_next(self,o,next);}.bind(item,self,o,next)});}});},submit:function(self,o,next){o.each(function(item){var t=item;var obj=$(t).select('form').first();if(obj){t=obj;}else{while(t&&t.tagName!='FORM'){t=t.parentNode}}
if(t){if(!t.action.include('__hibird_ajax')){if(t.action.include('?')){t.action=t.action+'&__hibird_ajax=1';}else{t.action=t.action+'?__hibird_ajax=1';}}
var methodName=t.method||"post";var params={};if(item.name&&item.value){params[item.name]=item.value;}
$(t).request({parameters:params,method:methodName.toLowerCase(),onSuccess:function(trans){do_next(self,o,next);},onFailure:function(trans){if(trans.responseText.length<300){hibird.notify(trans.responseText);}else{hibird.notify("Sorry, something went wrong",{kind:'error'});}},onComplete:function(trans){$(this).enable();$(this).select('button').each(function(b){b.disabled=false;});}.bind(t)});$(t).disable();$(t).select('button').each(function(b){b.disabled=true;});}});},propagate:function(o,next){}},select:function(self,filter){if(filter=='')return $A([self]);if(filter.include('+')){var result=[];$A(filter.split('+')).each(function(e){result=result.concat(scripty.select(self,e));});return $A(result);}
filter=filter.replace(/\!/g,'#');filter=filter.replace(/\*/g,':');filter=filter.replace(/\~/g,'.PARTIAL.');filter=filter.replace(/\%20/g,' ');var only_visible=false;if(filter.endsWith(':visible')){only_visible=true;filter=filter.substring(0,filter.length-8);}
var only_hidden=false;var result=null;if(filter.endsWith(':hidden')){only_hidden=true;filter=filter.substring(0,filter.length-7);}
var is_closest=false;if(filter.startsWith('^')){is_closest=true;filter=filter.substring(1);}
if(is_closest){var cur=self;result=$A([]);while(cur&&!result.any()){cur=cur.parentNode;result=$(cur).select(closest);}
return result;}else{result=$$(filter);}
if(only_visible){result=result.collect(function(item){if($(item).visible())return item});}
if(only_hidden){result=result.findAll(function(a){return!a.visible()});}
return result;},operate:function(self,url,ev){if(!url.include('#do='))return;var expr=url.split('#do=',2)[1];$A(expr.split(',')).each(function(stat){var filter=stat.substring(0,stat.indexOf(':'));var action=stat.substring(filter.length+1);var objs=scripty.select(self,filter);try{do_next(self,objs,action);}catch(e){}});return false;},hook:function(dom){var partials=dom.select('.PARTIAL');for(var i=0,len=partials.length;i<len;i++){var t=partials[i];if(t.title){var target=t.title;$(t).store('target',target);t.title='';}}
var objs=dom.select('.ajax');for(var i=0,len=objs.length;i<len;i++){var obj=objs[i];var actions=obj.className.split(' ');$A(actions).each(function(action){if(action.startsWith('on-')){var chunks=action.split('-',3);var event=chunks[1];var param=chunks[2];var url=obj[param];if(!obj.retrieve('ajax-'+event)){obj.store('ajax-'+event,url);obj[param]=url.split('#do=',1)[0];}else{url=obj.retrieve('ajax-'+event);}
obj.observe(event,function(obj,ev){var expr=obj.retrieve('ajax-'+this);scripty.operate(obj,expr,ev);if(!expr.include('propagate'))ev.stop();}.bind(event,obj));}});};manage_inline_form(dom);manage_textarea(dom);}};hibird.scripty=function(action){if(!action.include('#do=')){action='#do='+action;}
scripty.operate(document,action,null);};hibird.ready(function(){var url=window.location.href.toString();if(url.include('#do=')){window.location.href=url.split('#do=',1)[0];}
scripty.hook($$('body')[0]);});})();